...
// Based on Example 2 (block variant) from Puso & Laursen (2003)
...
int main(int argc, char* argv[])
{
  ...
  // Create DataStore
  std::string name = "block_on_slab";
  axom::sidre::DataStore datastore;
  smith::StateManager::initialize(datastore, name + "_data");

  // Construct the appropriate dimension mesh and give it to the data store
  // Using a 1x1 stiff block on a flexible slab
  // TODO: This would need a mesh file for the 1x1 block and slab
  ...
  std::shared_ptr<smith::Mesh> mesh = std::make_shared<smith::Mesh>(filename, "block_slab_mesh", 2, 0);
  ...
  // Slab: E = 1.0, nu = 0.3 (flexible)
  // Block: E = 1000.0, nu = 0.499 (stiff)
  ...
  // Slab: K = 1.0/(3*(1-0.6)) = 0.833, Block: K = 1000/(3*(1-0.998)) = 166667
  ...
  // Slab: G = 1.0/(2*(1+0.3)) = 0.385, Block: G = 1000/(2*(1+0.499)) = 333.6
  ...
  // Applied displacement to top of block
  mesh->addDomainOfBoundaryElements("top_of_block", smith::by_attr<dim>(12));
  auto applied_displacement = [](smith::tensor<double, dim>, double t) {
    constexpr double press_time = 0.2;  // Time to press down
  ...
  solid_solver.setDisplacementBCs(applied_displacement, mesh->domain("top_of_block"));

  // Add the contact interaction between slab and block surfaces
  auto contact_interaction_id = 0;
  std::set<int> slab_surface_attributes({6});
  std::set<int> block_surface_attributes({11});
  solid_solver.addContactInteraction(contact_interaction_id, slab_surface_attributes,
                                     block_surface_attributes, contact_options);
